...
### If the diff touches `.github/workflows/release.yml` (or before any release tag is pushed)

1. **Strip AppleDouble step exists between Export and Notarize** — the workflow has a step that runs `find "$APP" -name "._*" -delete` AND `find "$APP" -name ".__CodeSignature" -exec rm -rf {} +` AND `dot_clean -m "$APP"` AND `xattr -cr "$APP"` on the exported bundle. Without it, Dropbox-origin SPM dependencies (AlignedDesignSystem) leak macOS metadata files into the signed bundle.
2. **Re-sign step exists after Strip and before Notarize** — the workflow runs `codesign --force --deep --options runtime --timestamp --sign "Developer ID Application: …" --keychain "$KEYCHAIN_PATH"` on the cleaned bundle. Without it, the strip invalidates the signature on every nested bundle (Sparkle.framework, AlignedDesignSystem.bundle) and notarization either fails or produces a bundle Apple rejects.
3. **Verify Gatekeeper acceptance step exists after Staple, before Create distribution zip** — the workflow runs `spctl --assess --type execute -vv` AND `grep -q "accepted"` and exits non-zero if the bundle isn't accepted. **THIS IS THE KILL SWITCH.** Without it, a broken bundle can ship and CI will report success because earlier steps already uploaded the asset. Reference incident: v3.12.0 (2026-05-02) shipped with 160 AppleDouble files inside the signed bundle and broke Gatekeeper for every user — `xcrun stapler validate` passed but `spctl --assess` failed with "a sealed resource is missing or invalid".
4. **AppleDouble re-check after Verify** — the workflow asserts `find "$APP" -name '._*' | wc -l` returns `0` and exits non-zero otherwise. Catches any tool between re-sign and verify that reintroduces metadata. Belt-and-suspenders to the spctl gate.
5. **Notarize → Staple ordering preserved** — Strip + Re-sign go BEFORE Notarize (you must notarize the final-form bundle). Verify Gatekeeper goes AFTER Staple (the staple must be present for `xcrun stapler validate` to pass and for offline Gatekeeper checks). If you reorder these, Gatekeeper will fail on user machines even though CI passes.

### If shipping a release via the local-CLI build (the CI fallback)

Use this whenever the GitHub Actions release pipeline is bypassed and the notarized `.app` is built + signed on a Mac (the proven fallback used for v3.12.0 on 2026-05-02 and v3.12.1 on 2026-06-18, while CI hangs on the SwiftPM resolve deadlock). The repo lives inside a Dropbox-synced folder, which is the source of the bug class below.

1. **Strip + re-sign OUTSIDE the synced folder.** Copy the exported `.app` to a non-synced working dir (`/tmp/...`) — `ditto --noextattr --norsrc "<exported app>" "/tmp/.../<app>"` — and run every strip/re-sign/notarize/staple step on that `/tmp` copy. Dropbox (and iCloud Drive) re-apply `com.apple.FinderInfo` extended attributes to files inside the synced tree faster than you can strip them, so `codesign --verify --deep --strict` fails with "resource fork, Finder information, or similar detritus not allowed" even immediately after `xattr -cr`. The export can land in the repo's `build/`, but the signing work cannot. This is the xattr cousin of the AppleDouble (`._*`) bug from v3.12.0 — same root (Dropbox metadata), different symptom.
2. **Re-sign after ANY post-export change to the bundle.** If you patch a bundled resource into the `.app` after export (e.g. correcting `whats-new.json`), re-run `codesign --force --deep --options runtime --timestamp --sign "Developer ID Application: …"` and re-verify. `whats-new.json` is read from the app bundle via `Bundle.main` (`WhatsNewLoader.loadAll()`), so a post-archive edit to `Resources/whats-new.json` MUST be copied into the bundle and re-signed, or the shipped app shows stale notes even though the committed source is correct.
3. **Run the Gatekeeper gate by hand — there is no CI kill switch.** AFTER staple, `spctl --assess --type execute -vv "<app>"` MUST print `accepted` / `source=Notarized Developer ID`, and `find "<app>" -name '._*' | wc -l` MUST be 0. Do not zip / EdDSA-sign / release until both pass. This is the only thing standing between a broken bundle and Tiera's Mac.
4. **`sign_update` / `generate_keys` live at a hash-versioned DerivedData path that changes per resolve.** Never hardcode it — locate with `find ~/Library/Developer/Xcode/DerivedData -type f -name sign_update -path '*Sparkle*'`. (The path written in CLAUDE.md's Tool Locations is illustrative and goes stale.)
5. **Verify the live feed after publish.** After pushing both appcasts + creating the release, confirm the raw `SUFeedURL` actually serves the new version with the matching signature: `curl -s "<SUFeedURL>" | grep -E '<title>|edSignature'`, and that the enclosure download URL returns HTTP 200. raw.githubusercontent serves the root `appcast.xml` on `main` — if that doesn't show the new `<item>`, Sparkle will report "up to date" no matter what shipped.
